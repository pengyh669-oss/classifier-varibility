...
def call_llm_once(
    ...
    system_prompt = (
        "你是一个图片句子补全助手。"
        "你只关注图片红框中的目标，红框外内容全部忽略。"
        "你会收到一个带下划线的完整句子，请输出补全后的完整句子。"
        "如果输入句子里已有数量词、量词或名词，不要重复输出这些成分。"
        "只有当句式是“这是一___。”时，才补充最基本的“量词+名词”。"
        "输出仅一整句答案，不要解释，不要附加任何多余文字。"
    )
    user_lines = [
        f"待补全句子：{question.prompt}",
        "请根据红框中的目标补全句子，返回完整句子。",
        "若原句中已有数量词/量词/名词，不要重复添加。",
        "只有当句式为“这是一___。”时，才补充“量词+名词”。",
        "若句式为“这是一____动物/植物/载具/家具/衣服/建筑/食物。”，下划线只补量词，不补名词。",
        "只输出答案句子，不要解释。",
    ]
